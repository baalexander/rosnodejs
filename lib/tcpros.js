...
TCPROS.prototype.createPublisher = function() {
  ...
  portscanner.findAPortNotInUse(9000, null, hostname, function(error, port) {
    ...
    server.on('listening', function() {
      ...
      this.publisherUri = url.format({
        protocol : 'http'
      , hostname : address.address
      , port     : address.port
      });

      that.emit('listening', this.publisherUri);
    });
...
TCPROS.prototype.getPublisherUri = function(callback) {
  if (this.publisherUri) {
    callback(null, this.publisherUri);
  }
  else {
    this.on('listening', function(publisherUri) {
      callback(null, publisherUri);
    });
  }
};
