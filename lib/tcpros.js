...
var TCPROS = (function(){
  ...
  var TCPROS =   function (nodeName, topicName, messageType) {
    this.nodeName = nodeName;
    this.topicName = topicName;
    this.messageType = messageType;
  }

  TCPROS.prototype.createPublisher = function(callback){
    ...
    portscanner.findAPortNotInUse(9000, null, hostname, function(error, port) {
      ...
      var server = net.createServer(function(socket) {
        console.log("new socket" , socket.address() , "remote port" ,socket.remotePort);
        sockets.push(socket);
        ...
        socket.on('end', function(){ removeSocketFromSockets(socket);});
        socket.on('error', function(){ removeSocketFromSockets(socket);});
      ...
      console.log("listend on port :" , port , " host : ", hostname);
  ...
  TCPROS.prototype.createSubscriber = function(port, host, subscriber) {
    var that = this;
    console.log("create connection to port ", port);
  ...
  TCPROS.prototype.publish = function(message, callback) {
    console.log("tcp publish");
    console.log("tcp length", sockets.length);
    var self=this;
